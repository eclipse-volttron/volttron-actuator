...
def publish_agent(request, volttron_instance):
    ...
    assert fake_publish_agent.core.identity
    gevent.sleep(1)

    capabilities = {"edit_config_store": {"identity": PLATFORM_DRIVER}}
    volttron_instance.add_capabilities(fake_publish_agent.core.publickey, capabilities)
    gevent.sleep(1)
    ...
    fake_unit_testing_csv = """Point Name,Volttron Point Name,Units,Units Details,Writable,Starting Value,Type,Notes
...
OutsideAirTemperatureNoDefault,OutsideAirTemperatureNoDefault,F,-100 to 300,FALSE,,float,CO2 Reading 0.00-2000.0 ppm
SampleWritableFloatNoDefault,SampleWritableFloatNoDefault,PPM,1000.00 (default),TRUE,,float,Setpoint to enable demand control ventilation 
SampleLongNoDefault,SampleLongNoDefault,Enumeration,1 through 13,FALSE,,int,Status indicator of service switch
SampleWritableShortNoDefault,SampleWritableShortNoDefault,%,0.00 to 100.00 (20 default),TRUE,,int,Minimum damper position during the standard mode
SampleBoolNoDefault,SampleBoolNoDefault,On / Off,on/off,FALSE,,boolean,Status indidcator of cooling stage 1
SampleWritableBoolNoDefault,SampleWritableBoolNoDefault,On / Off,on/off,TRUE,,boolean,Status indicator
Heartbeat,Heartbeat,On / Off,on/off,TRUE,TRUE,boolean,Heart beat point
"""

    # Add platform driver configuration files to config store.
    fake_publish_agent.vip.rpc.call(CONFIGURATION_STORE, 'manage_store', PLATFORM_DRIVER, 'fake.csv',
                                    fake_unit_testing_csv, 'csv').get(timeout=10)

    fake_unit_testing_config = json.dumps({
        "driver_type": "fake",
        "driver_config": {},
    ...
    for i in range(4):
        ...
        fake_publish_agent.vip.rpc.call(CONFIGURATION_STORE, 'manage_store', PLATFORM_DRIVER, config_name,
                                        fake_unit_testing_config, 'json').get(timeout=10)

    # Install the Fake Driver library:
    cmd = ['pip', 'install', 'volttron-lib-fake-driver']
    process = Popen(cmd, env=volttron_instance.env,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
    result = process.wait()
    print(result)
    ...
    actuator_uuid = volttron_instance.install_agent(
        ...
        config_file=f"{Path(__file__).parent.resolve()}/actuator.config",
        vip_identity=PLATFORM_ACTUATOR,
        start=True)
...
def test_schedule_announce(publish_agent, volttron_instance):
    ...
    my_actuator_uuid = volttron_instance.install_agent(
        agent_dir=str(Path(__file__).parent.parent.resolve()),
        config_file=f"{Path(__file__).parent.parent.resolve()}/tests/actuator2.config",
    ...
    try:
        ...
        datetime1 = parse_timestamp_string(args_list1[4]['time'])
        datetime2 = parse_timestamp_string(args_list2[4]['time'])
...
def test_get_error_invalid_point(publish_agent):
    ...
    result_message = publish_agent.callback.call_args[0][5]
    assert result_message['type'] == 'volttron.driver.base.interfaces.DriverInterfaceError'
